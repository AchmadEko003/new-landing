...
<template>
  <ClientOnly>
    ...
    <UCarousel ref="carousel" v-slot="{ item, index }" :items="listImages" wheel-gestures drag-free :skip-snaps="true"
      class="w-full sticky top-[62px] bg-white z-50"
      :ui="{ item: 'basis-1/4 lg:basis-1/7 xl:basis-1/11', container: 'px-2 md:px-32' }">
      <div class="w-[4.5em] md:w-[5em] flex flex-col cursor-pointer justify-center items-center md:pb-3"
        v-if="index < ((listImages?.length || 0) - 1)">
        <UButton :color="undefined" variant="ghost" class="rounded-2xl cursor-pointer" @click="handleClick(index), emit('change', item.uid)">
          <NuxtImg v-if="item.name.toLowerCase() !== 'all'" provider="peponi"
            :src="`/unique-experience/512_${item.bannerName}.webp`" :alt="item.name" @contextmenu.prevent />
          <NuxtImg v-else src="/assets/all-icon.png" :alt="item.name" @contextmenu.prevent />
        </UButton>

        <h3 class="text-sm md:text-base font-bold text-center wrap-break-word md:max-w-[8em] xl:w-[8em]">{{ item.name }}</h3>

        <div class="mx-auto mt-1 rounded-full h-1 w-1/2 bg-emerald-600 transition-all duration-300 ease-in-out"
          v-if="selectCategory === index"></div>
      </div>
      <div v-else class="w-10 h-5"></div>
    </UCarousel>
  </ClientOnly>
</template>
